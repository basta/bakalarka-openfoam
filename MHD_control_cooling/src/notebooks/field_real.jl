...
# ╔═╡ d42a8e76-dc42-11ef-1a27-c50042ffbbbf
using  JLD, WGLMakie, LinearAlgebra, NearestNeighbors, PlutoUI, Interpolations

# ╔═╡ f583bdec-89a0-42f8-b402-944b3226b857
data = load("../../data/forceFields/B4.jld")

# ╔═╡ 5227333a-8235-4088-a8fd-580938b32fa3
dataE = load("../../data/forceFields/E1.jld")

# ╔═╡ ac506d05-6609-4a5d-8c81-f7fa725777dc
dataE2 = load("../../data/forceFields/E2.jld")

# ╔═╡ 92f914cf-4f6f-4192-8cf9-ba572f0e5dcc
VIS_STEP = 1

# ╔═╡ 9a3000e7-094d-4546-8b4b-70e8793e2fd9
grid = collect(eachrow(data["coords"].parent[1:VIS_STEP:end, :]))

# ╔═╡ 111de5bb-f98b-4108-875d-f3540ea953ca
gridE = collect(eachrow(dataE["coords"].parent[1:VIS_STEP:end, :]))

# ╔═╡ 0d05ffa5-f4db-4d11-9d74-3c4eec84dd5a
vectors = collect(zip(data["Bx"], data["By"], data["Bz"]))[1:VIS_STEP:end]

# ╔═╡ a62384d6-ea4b-40e1-acdc-7226eb5f4a8e
vectorsE = collect(zip(dataE["Ex"], dataE["Ey"], dataE["Ez"]))[1:VIS_STEP:end]

# ╔═╡ 0559ff42-5eae-4168-ae7e-2c82e29c10a5
ps = [Point3f(x[1], x[2], x[3]) for x in gridE]

# ╔═╡ 6536a2e5-a675-434d-bc42-0ff8e528e0a0
ns = [Vec3f(x[1], x[2], x[3]) for x in vectorsE]

# ╔═╡ 2c4a6056-d9ca-4333-a2da-e4537208442f
begin
	# scene = Scene()
	# arrows(ps, ns, arrowcolor = :blue, arrowsize = 0.01, lengthscale = 1, linecolor = :black)
	fig = Figure()
	ax = Axis3(fig[1,1])
	color_data = [norm(x) for x in vectors]
	clamp!(color_data, 0, 0.4)
	color_data = log.(2, color_data)
	sc = scatter!(ax, [x[1] for x in grid], [x[2] for x in grid], [x[3] for x in grid], color=color_data, alpha=0.7)
	Colorbar(fig[1, 2], sc, label = "strength magnitude")
	fig
end

# ╔═╡ 23642256-9752-441c-82b6-5da8d8fba839
typeof(vec(vectors))

# ╔═╡ 9bb6984f-28a8-4196-9944-60a710ecfa91
vectors[4]

# ╔═╡ 3c7c69ad-814f-49aa-95f9-c84aa5d5a7a2
begin
	tree = KDTree(data["coords"].parent'[:, 1:end])
end

# ╔═╡ f5b52d5e-1008-4ca7-82c4-3f29c0dccd60


# ╔═╡ 7716d315-9378-4cab-a6a0-ddd2a45a3bc4
...
begin 
	...
	for x in X
		for y in Y
			push!(grid_vecs, (x,y,Z)) 
		end
	end
	out_grid = stack(grid_vecs)
end

# ╔═╡ 9a3a948a-d928-4d86-827a-7ed56f37b213
idxss, distss = knn(tree, out_grid[:, :], 5)

# ╔═╡ ba13a2ad-baaa-4e7b-b7c4-2bca19ffc51d
...
# ╔═╡ 2979b827-a69b-4064-a463-80da6d8497bc


# ╔═╡ c3e0baec-895a-40a0-9e8a-0e622797a025
begin
	poses = []
	vecs = []
	for (i, idxs) in enumerate(idxss)
		...
		for (dist, i_vec) in zip(dists, i_vecs)
			...
			w_sum += w_i
			# i_vec = cross(collect(i_vec), [1;0;0])
			vec2 += Vec2f(i_vec[1], i_vec[2])*w_i
		end
		
		vec2 = vec2 / w_sum
		
		push!(vecs,vec2)
		push!(poses, Point2f(pos[1],pos[2]))
	end
	f = Figure(size = (800, 800))
	Axis(f[1, 1], backgroundcolor = "black")
	strength = log.(10, norm.(vecs))
	arrows(poses, vecs, arrowcolor = strength, arrowsize = 10, lengthscale = 0.05, linecolor = strength)
end

# ╔═╡ 634f74eb-1f8f-4d01-93cd-a4a10523d88b
begin
	treeE = KDTree(dataE["coords"].parent'[:, 1:end])
	idxssE, distssE = knn(treeE, out_grid[:, :], 5)
	begin
		posesE = []
		vecsE = []
		for (i, idxs) in enumerate(idxssE)
			...
			push!(vecsE,vec2)
			push!(posesE, Point2f(pos[1],pos[2]))
		end
		fE = Figure(size = (800, 800))
		Axis(f[1, 1], backgroundcolor = "black")
		strengthE = norm.(vecsE)
		arrows(posesE, vecsE, arrowcolor = strengthE, arrowsize = 10, lengthscale = 0.0002, linecolor = strengthE)
	end
end

# ╔═╡ c8eb9bf6-9d17-4088-93c1-4b10d758fef6
begin
	treeEl = KDTree(dataE["coords"].parent'[:, 1:end])
	treeEl2 = KDTree(dataE2["coords"].parent'[:, 1:end])
	
	B = interpolate_tree(tree, vectors, out_grid)
	E = interpolate_tree(treeEl, vectorsE, out_grid)
	E2 = interpolate_tree(treeEl2, vectorsE, out_grid)
	
	posesF = []
	vecsF = []
	for (E1i,E2i,Bi,Pi) in zip(eachcol(E), eachcol(E2),eachcol(B),eachcol(out_grid))
		Ei = E1i .+ E2i
		push!(posesF, Point2f(Pi[1], Pi[2]))
		push!(vecsF, Vec2f(cross(Ei, Bi)[1:2]...))
	end
...
manifest_format = "2.0"
project_hash = "91718e2cc904a5b29dedc2145fb7586149690aa1"
...
git-tree-sha1 = "b2132bacf072eae4da0cd75a260d0e79c04530ff"
