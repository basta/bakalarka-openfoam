...
# ╔═╡ 5fa8ce9a-1005-11f0-2db7-65b1e3ce7a85
using LinearAlgebra, JLD2, Statistics

# ╔═╡ 0c95fbef-f323-46a7-bc08-6b6ff63ce321
using WGLMakie

# ╔═╡ 8c96b15c-d326-4075-aa69-0d8f9c1fbc32
A = rand(5,5)

# ╔═╡ 7d20ef44-af68-435b-94e1-9085a15736b3
pinv(A)

# ╔═╡ dc97ce9e-49d6-4b0b-9498-aa0952462e15
U, s, Vt = svd(A)

# ╔═╡ 47af3564-6445-4c7d-9d06-d7c9b777b1f8
U*diagm(s)*Vt'

# ╔═╡ 9a8e1c30-bc0d-4ed9-a084-00ea9d3d2355
begin
	s_modified = s[:]
	s_modified[4:end] .= 0 
end

# ╔═╡ 163090e2-1cdd-44a8-9158-249633daa625
begin
	r = 5
	A_inv_reduced = Vt[:, 1:r]*inv(diagm(s[1:r]))*(U')[1:r, :]
end

# ╔═╡ 578d7e7c-6af6-48ad-b6e6-d6e7cb480edd
A*pinv(A)

# ╔═╡ f4fd7c32-1c79-44cb-9463-04b0b6f609fa
A*A_inv_reduced

# ╔═╡ f741873f-c7d7-4269-ad2b-c62346e11914
X_data = jldopen("../../data/dataset-live.jld2")["dataset"][2];

# ╔═╡ 368951d3-df42-42e0-8d3c-8f563aa12fe0
function reduced_pinv(A, r)
	U, s, Vt = svd(A)
	return Vt[:, 1:r]*inv(diagm(s[1:r]))*(U')[1:r, :], s
end

# ╔═╡ 002ce27c-3c83-49eb-8ecb-18c27e645243
begin
	X_prev = X_data[:, 1:end-1]
	X_next = X_data[:, 1:end-1]
end

# ╔═╡ 1e90ee4d-fa24-4a29-990b-fd509098fb8e
function standardize_data(data::AbstractArray{Float64, 2}; no_std=false)
...
# ╔═╡ 0877cf81-16b4-44dd-ad44-0e51a23b66c1
X_prev_norm, _, _ = standardize_data(X_prev; no_std=true)

# ╔═╡ e8fbfe8c-839f-4a34-9621-b9f09e3589bf
X_p, s_p = reduced_pinv(X_prev_norm, 50)

# ╔═╡ 7640d059-5a42-4a6c-a97a-071c59faf700
s_p

# ╔═╡ 4ec59cf7-3b75-4ce6-9e29-26c8175113d5
begin
	total_energy = [sum(map(x->x^2, s_p[1:end]))[1]]
	total_energy = vec(total_energy)
	energies = [sum(map(x->x^2, s_p[1:r]))/total_energy for r in 1:101] 
...
# ╔═╡ 55602218-9bb9-4519-9975-3449cd540609
begin
	fig = Figure()
	ax = Axis(fig[1,1])
	lines!(X_prev_norm[5, 2000:2100],label="full")
	lines!(pinv(X_p)[5, 2000:2100],label="reduced")
	axislegend(ax)
	fig
end
...
manifest_format = "2.0"
project_hash = "bd2953c6e9a57423609f57ccf5e25fae0dc054c9"
