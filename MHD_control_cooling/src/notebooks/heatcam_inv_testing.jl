...
# ╔═╡ e798eb92-9c52-4bab-a6d9-a49e8e1fcdc3
using LinearAlgebra, JLD2, Statistics, CairoMakie, ControlSystems, RollingFunctions, SimplePlutoInclude, Revise, PlutoUI,RollingFunctions, IterativeSolvers , LowRankApprox,  EquivariantOperators   

# ╔═╡ c63058ce-bad7-4c6a-ba46-1aa0ee0d10c6
@plutoinclude "../thermal/thermal_inverse.jl"        

# ╔═╡ b1a10476-122e-11f0-3f7c-c15737758c76
begin
	X_data = jldopen("../../data/dataset-live.jld2")["dataset"][2]; 
	X_data = X_data[1:400,:]; 
end

# ╔═╡ 4ff13752-4200-4a35-bcf3-6355810dccfd
begin
	IDX = 3000
	DTt_field = X_data[:, IDX] - X_data[:, IDX-1] 
	DTt_field = reshape(DTt_field, 20,20)
	Ux, Uy = calculate_velocity_from_temperature(reshape(X_data[:, IDX], 20, 20), DTt_field, 0.1, 0.1)  
end

# ╔═╡ cb18a3eb-e400-4ddb-9ec3-6fae7f1cbcb1
heatmap(reshape(DTt_field, 20, 20)) 

# ╔═╡ 6ecf8663-b23d-412b-b902-b79780e63a26
heatmap(sqrt.(Ux.^2 + Uy.^2))

# ╔═╡ 3d2dda3a-38fe-4391-a2fe-3c15320a12da
let
    # --- Configuration --- 
    # Assuming Ux and Uy are 20x20 matrices from the previous calculation
    # If they are not available in this scope, you might need to recalculate them here
    # For example:
    # T_input = reshape(X_data[:, 1], 20, 20)
    # dx_val = 0.1
    # dy_val = 0.1
    # Ux, Uy = calculate_velocity_from_temperature(T_input, dx_val, dy_val)

    N, M = size(Ux) # Get dimensions (should be 20, 20)
    dx = 0.1        # Grid spacing in x used in the calculation
    dy = 0.1        # Grid spacing in y used in the calculation

    # --- Prepare Grid Coordinates ---
    # Create coordinates for the center of each cell
    x_coords = [(i - 0.5) * dx for i in 1:N, j in 1:M]
    y_coords = [(j - 0.5) * dy for i in 1:N, j in 1:M]

    # --- Calculate Magnitudes (for coloring) ---
    magnitudes = sqrt.(Ux.^2 .+ Uy.^2)

    # --- Create Plot ---
    fig = Figure(size = (600, 600)) # Adjust figure size if needed
    ax = Axis(
        fig[1, 1],
        title = "Velocity Field (Quiver Plot)",
        xlabel = "X coordinate",
        ylabel = "Y coordinate",
        aspect = DataAspect() # Ensures dx and dy units look square
    )

    # Create the quiver plot using arrows!
    # We need to flatten the 2D arrays into 1D vectors for arrows!
    arrow_plot = arrows!(
        ax,
        vec(x_coords),    # Flattened x positions
        vec(y_coords),    # Flattened y positions
        vec(Ux)./maximum(Ux),          # Flattened x-components of vectors
        vec(Uy)./maximum(Uy),          # Flattened y-components of vectors
        arrowsize = 12,    # Size of the arrowhead (adjust as needed)
        lengthscale = 0.1, # Scales the length of the arrows (adjust as needed)
        arrowcolor = vec(magnitudes), # Color arrows by magnitude
        linecolor = vec(magnitudes),  # Color lines by magnitude
        colormap = :viridis # Choose a colormap (e.g., :viridis, :plasma)
    )

    # Add a colorbar to show the magnitude scale
    Colorbar(fig[1, 2], arrow_plot, label = "Velocity Magnitude")

    # Display the figure
    fig # Or just `fig` in environments like Pluto/VS Code Plot Pane
end
...
manifest_format = "2.0"
project_hash = "6a60618736a5628b6d5f6cdcc7ad6f319a09b3a4"
...
git-tree-sha1 = "c507ca86353b28a2817618def22d5ac63f72a53c"
...
[[deps.EquivariantOperators]]
deps = ["DSP", "FFTW", "Functors", "ImageFiltering", "LinearAlgebra", "Memoize", "NearestNeighbors", "Random", "StaticArrays", "UnPack", "Zygote"]
git-tree-sha1 = "f1f1ba4a0e35b4b0fe084b754a46c9d388c81002"
