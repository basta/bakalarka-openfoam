...
# ╔═╡ 56c98fe0-1266-11f0-29f7-2be8289bd120
using LinearAlgebra, Statistics, BenchmarkTools, SimplePlutoInclude, ProperOrthogonalDecomposition, StatsBase, WGLMakie , JLD2

# ╔═╡ cabb88a7-832c-4d0d-b422-38e5c7966ef3
X = rand(10, 30000)

# ╔═╡ fe8b8b98-fc8f-423b-9dc1-cdc2f1988390
X_real = jldopen("../../data/dataset-live.jld2")["dataset"][2][1:400, :];

# ╔═╡ 39c5bcc7-bebf-4b45-962b-8757fca9f6e2
@plutoinclude "../dim_reduction.jl" 

# ╔═╡ 111c07d0-90f4-4602-bafe-9937a4ed0d10
begin
	X_next = X[:, 2:end]
	X_prev = X[:,  1:end-1] 
end

# ╔═╡ ef16c0b8-ee84-494e-837e-a21d57f5fd58
begin
	# @benchmark X_next * pinv(X_prev) 
	# error = mean(abs.(X_next - A_1 * X_prev))
	# md"avg error is $error"  
end

# ╔═╡ 7312b908-745a-4a33-9269-bf1641f076d8
begin
	function fast_dmd()
		G = 1/size(X,2) * X_next * X_prev'
		H = 1/size(X,2) * X_prev * X_prev' 
		A_2 = G*pinv(H)
	end
	# @benchmark fast_dmd()
	# error2 = mean(abs.(X_next - A_2 * X_prev))
	# md"avg error is $error2" 
end

# ╔═╡ 00bbc366-ee08-4db6-ac83-20376aa53ae1
svd(X)

# ╔═╡ 6bba2a80-bb26-4829-b706-59125bbbc95a
# basis, s = POD(X, subtractmean=true)
@benchmark POD(X)

# ╔═╡ 7d1d5511-7c0c-4429-b629-c757a1f3c98a
@benchmark svd(X)

# ╔═╡ 6db642d0-d56b-48cc-a02d-dafdd8d255d0
cor_res = autocor(X_real, demean=true)

# ╔═╡ e69728bf-1a91-49e6-9f88-22f0266f733f
begin
	fig = Figure()
	ax = Axis(fig[1,1])
	for i in 10
		lines!(ax, cor_res[10, 1:10000])
	end
	fig
end

# ╔═╡ b2aaf066-3f0d-4c83-beff-1f52e3aeb831
let
	cor_res = autocor(X_real[5,:], demean=true)
	fig, ax, _ = lines!(cor_res)
	fig
end
...
ProperOrthogonalDecomposition = "78421dfc-ac16-5d69-8300-c22153e3d0b6"
...
JLD2 = "~0.5.11"
ProperOrthogonalDecomposition = "~1.0.4"
...
manifest_format = "2.0"
project_hash = "da78f4927b40ef83856bde04ef27f5c595d9ed7b"
...
[[deps.ProperOrthogonalDecomposition]]
deps = ["LinearAlgebra", "StableRNGs", "Statistics"]
git-tree-sha1 = "29ce1bd02d909df87c078b573c713c4888c1d968"
uuid = "78421dfc-ac16-5d69-8300-c22153e3d0b6"
version = "1.0.4"
