...
# ╔═╡ bfcb5074-58d9-41c2-a50f-1ca474e4c016
using JLD2, DataDrivenDiffEq, ModelingToolkit, OrdinaryDiffEq, DataDrivenSparse, LinearAlgebra, DataDrivenDMD , WGLMakie, SimplePlutoInclude, IterativeSolvers, LowRankApprox

# ╔═╡ 49062d29-dd08-4806-842d-e98de6d801d2
begin
	@plutoinclude "../pinverses.jl"    
	@plutoinclude "../sys_models.jl"  
end

# ╔═╡ da23380e-1217-11f0-30d6-f3d565d6ab5f
begin
	CUTOFF = 1000
	DIM_CUTOFF = 400
	X_data = jldopen("../../data/dataset-live.jld2")["dataset"][2];
	X_data = X_data[1:DIM_CUTOFF,1:CUTOFF]; 
	 
	U_data = jldopen("../../data/dataset-live.jld2")["dataset"][1][:,1:CUTOFF];
	N_states = size(X_data,1)  
	N_inputs = Int((size(U_data,1)/2)^2)  
end

# ╔═╡ d18a1135-0ddb-4e57-b701-bd3bdf7c1e71
U_data16 = let
	u = U_data[:,1000]
	cols = []
...
# ╔═╡ ce54c2ba-8961-4be3-ba1c-da0e78b8c82a


# ╔═╡ 3c91a8dc-fb40-40fa-a35f-657989512482
begin
	U, s, Vt = svd(standardize_data(X_data)[1]) 
	total_energy = sum([k^2 for k in s])
	partial_energy = cumsum(s.^2)
	lines(partial_energy./total_energy)
	lines(log.(10, s))
end;

# ╔═╡ de0b4ba7-2683-47f4-b826-2dff5c092713
for (i, p) in enumerate(partial_energy./total_energy)
	# println("$i: $p")
end;

# ╔═╡ 7cac2bba-6cd3-4c64-8b8b-c418bcdd5498
begin
	X_prev = X_data[:, 1:end-1]
	X_next = X_data[:, 2:end]
	U_dmd = U_data[:, 1:end-1]

	prob = DiscreteDataDrivenProblem(X_data, (0:size(X_data,2)-1)./2, U_data)
	alg = DMDSVD(5) 
end

# ╔═╡ cd831b4d-8bb2-48cf-9b05-ef56127eba6d
res = solve(prob, alg,options=DataDrivenCommonOptions(verbose=true))  
...
[compat]
DataDrivenDMD = "~0.1.1"
DataDrivenDiffEq = "~1.6.0"
DataDrivenSparse = "~0.1.2"
IterativeSolvers = "~0.9.4"
JLD2 = "~0.5.11"
LowRankApprox = "~0.5.5"
ModelingToolkit = "~9.65.0"
...
manifest_format = "2.0"
project_hash = "86635cb5380a47a22a123f4043c3759bb1ffd770"
...
[[deps.DataDrivenDMD]]
deps = ["DataDrivenDiffEq", "LinearAlgebra"]
git-tree-sha1 = "cff2a47ef06c6d63567fc8346347f48360631996"
