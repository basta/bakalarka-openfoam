...
# ╔═╡ 160816b8-0201-11f0-199d-4f5014724763
using CairoMakie, JLD2, Statistics

# ╔═╡ 7da26e06-0857-4cf2-90c9-e395cd151501
let
	
	X = jldopen("../../data/dataset-long.jld2")["dataset"][2]
	U = jldopen("../../data/dataset-long.jld2")["dataset"][1]

	fig = Figure(resolution = (1200, 800), fontsize=20)


	ax = Axis(fig[1, 1], 
	    title = "Simulated T₁ temperaure",
	    xlabel = "Time [s]",
	...
	lines!(ax, X[1, 1000:1500], label="T₁")
	ax = Axis(fig[1, 1], 
	    title = "Simulated T₁ temperaure",
	    xlabel = "Time [s]",
	    ylabel = "Mean input",
	...
	ylims!(ax, -5, 15)
	lines!(ax, U[1, 1000:1500], color=:red, linewidth=2, label="Input avg")
	lines!(ax, [0], color=:blue, linewidth=2, label="T₁ value")
...
manifest_format = "2.0"
project_hash = "f622363a4b0cb33caf0f38291da0ee17abaeeaaa"
