...
function initialize_controller()
    ...
    # No need for params, specific_params, controller_specific_params extraction here
    # The constructor DenseMPCController(config) will handle pulling its own params

    @info "Initializing controller of type: $controller_type"

    try
        if controller_type == "Identification"
            params = get(config, "controller", Dict())
            T_stabilize = get(params, "T_stabilize", 100.0)
            ...
            if startswith(request, "REQUEST_FORCES_")
                ...
                if m !== nothing
                    ...
                    if current_controller !== nothing
                        local input::Vector{Float64} # Control input vector
                        try
                            # Remove the hardcoded lines below:
                            ...
                            # Call the method on the current controller instance
                            ...
                            # Determine nu based on initialized controller if possible, otherwise hardcode/config
                            default_nu = 8 # Assuming 8 inputs as fallback/default
                            default_nu = current_controller.mpc_data.nu # Try to get nu
                            input = zeros(default_nu)
                        end
