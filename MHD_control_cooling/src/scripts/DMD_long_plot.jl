...
3. A publication-quality figure showing one long segment of original data, with multiple simulations
   (ABC and DMDc) starting at various points along this segment, illustrating their deviation.
   (defined by DEVIATION_PLOT_CONFIG).

All figures compare predictions against the selected actual data using Plots.jl.
...
# Simulation and Plotting Parameters for Deviation Plot
const DEVIATION_PLOT_CONFIG = Dict(
    :base_start_index => 500,    # Start index of the long actual data segment
    :base_length => 2500,         # Length of the long actual data segment
    :sim_start_offsets => [0, 400, 800, 1200, 1600], # Offsets relative to base_start_index for new sims
    :sim_length => 300            # Length of each branching simulation
)
...
default(
    ...
    linewidth=1.5,
    markersize=3,
    legendfontsize=10, # Increased for better readability on single plots
    tickfontsize=8,
    guidefontsize=10,
    titlefontsize=12, # Slightly increased for main titles
...
function load_all_data(model_path::String, timeseries_path::String)
    ...
    try
        jldopen(timeseries_path, "r") do file
            ...
            # Specific U_data transformation if N_U is 16 and raw U_data has 8 features
            if N_U == 16 && size(U_data_raw, 1) == 8
                ...
                for i in 1:size(U_data_transformed, 2)
                    if size(U_data_raw, 1) >= 8 # Should always be true if first check passed
                ...
                U_data = U_data_raw # Use raw U_data if no transformation needed or if dimensions already match N_U
            end
    ...
    if size(U_data, 1) != N_U # Check after potential transformation
...
function create_abc_init_state(X::AbstractMatrix, U::AbstractMatrix, start_time::Int, delay::Int, mean_x::AbstractVecOrMat, N_X::Int, N_U::Int)
    ...
    if delay > 0 # U history is only needed if delay > 0
        u_min_hist_idx = start_time - delay
        u_max_hist_idx = start_time - 1 # U history is up to t-1
        if u_min_hist_idx < 1 || u_max_hist_idx >= start_time # u_max_hist_idx should be less than start_time
            # Corrected condition: u_max_hist_idx needs to be within U's bounds
            if u_min_hist_idx < 1 || u_max_hist_idx > size(U, 2) || u_max_hist_idx < u_min_hist_idx
...
function create_orig_init_state(X::AbstractMatrix, U::AbstractMatrix, start_time::Int, delay::Int, mean_x::AbstractVecOrMat, N_X::Int, N_U::Int)
    state_len = N_X * (delay + 1) + N_U * (delay + 1) # Omega includes current U placeholder
    init_omega = zeros(state_len)

    min_required_x_time = start_time - delay # For X_history
    if min_required_x_time < 1 || start_time > size(X, 2)
    ...
    # U history for Omega state (U(t-1)...U(t-delay))
    if delay > 0
        # U_history goes from start_time-1 down to start_time-delay
        min_required_u_hist_time = start_time - delay
        max_required_u_hist_time = start_time - 1
        if min_required_u_hist_time < 1 || max_required_u_hist_time > size(U, 2) || max_required_u_hist_time < min_required_u_hist_time
            error("Cannot create initial state for original model at time $start_time with delay $delay: requires U history data in range [$(min_required_u_hist_time), $(max_required_u_hist_time)], available U=[1, $(size(U,2))]")
        end
    end
    # Note: The current U(t) slot in Omega is filled during simulation step.

    # Fill X part of Omega: X(t), X(t-1), ..., X(t-delay)
    for i in 0:delay
        idx_start = i * N_X + 1
        idx_end = (i + 1) * N_X
    ...
    # Fill U part of Omega: U(t) placeholder, U(t-1), ..., U(t-delay)
    # U(t) placeholder (first N_U elements in U part of Omega) is left zero or handled by sim.
    # The simulation function `simulate_orig_model` expects to fill U(t) (current input).
    # We fill U_history: U(t-1) down to U(t-delay)
    u_embed_start_idx_in_omega = N_X * (delay + 1) + 1 # Start of U block in Omega
    # The first N_U slots are for current U, filled by simulator.
    # The next N_U*delay slots are for U_history.
    if delay > 0
        for i in 1:delay # U(t-i)
            # Offset for U(t-i) within the U-history part of Omega
            # U(t-1) is at u_embed_start_idx_in_omega + N_U
            # U(t-delay) is at u_embed_start_idx_in_omega + N_U*delay
            actual_idx_start = u_embed_start_idx_in_omega + i * N_U
...
function simulate_orig_model(initial_omega::AbstractVector,
    ...
    x_aug_len = n_x * (delay + 1) # x(t), x(t-1)...x(t-d)
    u_aug_len = n_u * (delay + 1) # u(t), u(t-1)...u(t-d)

    # Index for current input u(t) within Omega vector
    ...
    for k_sim_step in 1:sim_length
        # Update current input u(t_k) in Omega
        ...
        # Predict next state x(t_k+1)
        x_next_centered = M_dmdc * current_omega
        states_out_centered[:, k_sim_step] = x_next_centered

        # Update Omega for next step:
        # Shift X history: x(t-d) is dropped, x(t-d+1) becomes x_new(t-d), ..., x(t) becomes x_new(t-1)
        if x_aug_len > n_x # if delay > 0 for X
            current_omega[n_x+1:x_aug_len] = current_omega[1:x_aug_len-n_x]
        end
        # Newest X state (predicted one) becomes x(t) for next Omega
        current_omega[1:n_x] = x_next_centered

        # Shift U history: u(t-d) is dropped, ... u(t) becomes u_new(t-1)
        # The u(t) slot (first N_U in U part) will be overwritten by next input.
        # History starts after the current u(t) slot.
        if u_aug_len > n_u # if delay > 0 for U
            # u_history_target_start = u_current_start_idx_in_omega + n_u # Start of u(t-1) slot
            # u_history_source_start = u_current_start_idx_in_omega      # Start of u(t) slot (which was just used)
            # len_to_shift = u_aug_len - n_u - n_u # We shift (delay-1) blocks of N_U, from U(t) up to U(t-delay+1)
            # to become U(t-1) up to U(t-delay)
            if delay > 0 # only shift if there is history
                for i_delay_u = delay:-1:1 # Shift u(t-i+1) to u(t-i)
                    source_u_idx_start = x_aug_len + (i_delay_u - 1) * n_u + 1
                    dest_u_idx_start = x_aug_len + i_delay_u * n_u + 1
                    # Corrected line: ensured space before the trailing '1' in the second index range
                    current_omega[dest_u_idx_start:dest_u_idx_start+n_u-1] = current_omega[source_u_idx_start:source_u_idx_start+n_u-1]
                end
            end
        end
        # The current input u(t) (at current_omega[u_current_start_idx_in_omega:u_current_end_idx_in_omega])
        # is already set from inputs[:, k_sim_step] at the beginning of the loop.
        # For the *next* iteration, this slot will be filled with inputs[:, k_sim_step+1].
        # The history has been shifted correctly to reflect that the u(t) just used becomes u(t-1) for the next step.
    end
...
function plot_single_comparison_subplot!(p::Plots.Plot,
    ...
    if !is_short_term_plot
        ...
        plot!(p[subplot_idx], grid=true) # Ensure grid lines are on
    end
end
...
function main()
    ...
    if num_long_term_plots > 0
        ...
        long_term_combined_plot = plot(layout=plot_layout_long, size=(fig_width_long, fig_height_long), legend=false) # legend handled per subplot
        ...
        for sim_start_time in START_INDICES
            ...
            if min_hist_time < 1
                annotate!(long_term_combined_plot[long_term_subplot_counter], (0.5, 0.5), text("Skipped: Start too early for history", :orange, :center, 8))
            ...
            if effective_sim_len <= 0 # Should be caught above, but as a safeguard
            ...
            # Ensure U_data and X_data cover the simulation period
            if maximum(sim_indices) > size(U_data, 2) || maximum(sim_indices) > size(X_data, 2)
                annotate!(long_term_combined_plot[long_term_subplot_counter], (0.5, 0.5), text("Error: Sim indices exceed data bounds", :red, :center, 7))
                title!(long_term_combined_plot[long_term_subplot_counter], "Start = $sim_start_time")
                println("Error: Simulation indices for start $sim_start_time exceed U_data or X_data bounds.")
                continue
            end
    ...
    if num_short_term_plots > 0
        ...
        fig_width_short = max(1200, 250 * grid_cols_short) # Ensure reasonable width
        fig_height_short = max(900, 200 * grid_rows_short) # Ensure reasonable height
        short_term_combined_plot = plot(layout=plot_layout_short, size=(fig_width_short, fig_height_short), legend=:topright) # Global legend
        ...
        for sim_start_time_short in SHORT_TERM_START_INDICES
            ...
            if maximum(sim_indices_short) > size(U_data, 2) || maximum(sim_indices_short) > size(X_data, 2)
                annotate!(short_term_combined_plot[short_term_subplot_counter], (0.5, 0.5), text("Error: Sim indices exceed data bounds", :red, :center, 5))
                println("Error: ST Simulation indices for start $sim_start_time_short exceed U_data or X_data bounds.")
                continue
            end
            ...
            plot_single_comparison_subplot!(short_term_combined_plot, short_term_subplot_counter,
                ...
                short_term_subplot_counter == 1, is_short_term_plot=true) # Add legend labels only for the first subplot
        end
        ...
        plot!(short_term_combined_plot, plot_title="Short-Term DMDc vs ABC (Delay=$delay, State=$(original_state_label_str), Pred Len=$(SHORT_TERM_SIM_LEN))",
            plot_titlefontsize=14, top_margin=10Plots.mm)
        ylabel!(short_term_combined_plot, "State $(original_state_label_str) (Global Time)") # Global Y label
    ...
    # --- Generate Deviation Comparison Figure ---
    println("\n\n--- Generating Deviation Comparison Figure ---")
    if length(DEVIATION_PLOT_CONFIG[:sim_start_offsets]) > 0
        base_start = DEVIATION_PLOT_CONFIG[:base_start_index]
        base_len = DEVIATION_PLOT_CONFIG[:base_length]
        sim_offsets = DEVIATION_PLOT_CONFIG[:sim_start_offsets]
        branch_sim_len = DEVIATION_PLOT_CONFIG[:sim_length]

        base_end = base_start + base_len - 1
        if base_start < 1 || base_end > TOTAL_SAMPLES || base_len <= 0
            println("Error: Invalid base segment definition for deviation plot (start=$base_start, len=$base_len). Max samples: $TOTAL_SAMPLES. Skipping deviation plot.")
        else
            deviation_plot = plot(size=(1200, 700), legend=:outertopright) # Larger plot, legend outside if possible or :best

            # Plot the long base segment of actual data
            base_time_indices = base_start:base_end
            actual_base_segment = X_data[PLOT_STATE_IDX, base_time_indices]
            plot!(deviation_plot, base_time_indices, actual_base_segment,
                label="Actual Data (State $(original_state_label_str))", color=:black, linewidth=2.5)

            println("Plotting base actual data from $base_start to $base_end for state $original_state_label_str.")

            for (sim_idx, offset) in enumerate(sim_offsets)
                sim_start_time = base_start + offset
                println("\n--- Processing Deviation Sim, Start Time (Global): $sim_start_time (Offset: $offset) ---")

                effective_branch_sim_len = branch_sim_len
                min_hist_time = sim_start_time - delay
                max_sim_time = sim_start_time + effective_branch_sim_len - 1

                if min_hist_time < 1
                    println("Skipping deviation sim at $sim_start_time: Start too early for history (requires up to $min_hist_time).")
                    continue
                end
                if max_sim_time > TOTAL_SAMPLES
                    effective_branch_sim_len = TOTAL_SAMPLES - sim_start_time + 1
                    if effective_branch_sim_len <= 0
                        println("Skipping deviation sim at $sim_start_time: Start effectively beyond data range after length adjustment.")
                        continue
                    end
                    println("Adjusting deviation sim length to $effective_branch_sim_len for start $sim_start_time.")
                end
                if effective_branch_sim_len <= 0
                    println("Skipping deviation sim at $sim_start_time: Zero effective simulation length.")
                    continue
                end

                sim_indices = sim_start_time:(sim_start_time+effective_branch_sim_len-1)
                if maximum(sim_indices) > size(U_data, 2) || maximum(sim_indices) > size(X_data, 2)
                    println("Error: Deviation sim indices for start $sim_start_time exceed U_data or X_data bounds. Skipping.")
                    continue
                end
                sim_inputs = U_data[:, sim_indices]
                # actual_data_for_sim_period = X_data[:, sim_indices] # Not directly plotted as 'actual' here, but used for initial state

                local init_state_abc_dev, init_state_orig_omega_dev
                try
                    init_state_abc_dev = create_abc_init_state(X_data, U_data, sim_start_time, delay, mean_X_vec, N_X, N_U)
                    init_state_orig_omega_dev = create_orig_init_state(X_data, U_data, sim_start_time, delay, mean_X_vec, N_X, N_U)
                catch e
                    println("Error creating initial state for deviation sim at $sim_start_time: $e. Skipping.")
                    continue
                end

                println("Running deviation sims for start $sim_start_time (len $effective_branch_sim_len)...")
                states_abc_aug_dev = simulate_abc_model(A_full, B_full, init_state_abc_dev, sim_inputs, effective_branch_sim_len)
                states_abc_dev = states_abc_aug_dev[1:N_X, :] .+ mean_X_vec

                states_orig_centered_dev = simulate_orig_model(init_state_orig_omega_dev, sim_inputs, M_dmdc, effective_branch_sim_len, delay, N_X, N_U)
                states_orig_dev = states_orig_centered_dev .+ mean_X_vec

                # Plotting the simulated segments
                plot!(deviation_plot, sim_indices, states_abc_dev[PLOT_STATE_IDX, :],
                    label=(sim_idx == 1) ? "ABC Model" : "", # Label only for the first segment
                    color=:red, linestyle=:dash, linewidth=1.2)

                plot!(deviation_plot, sim_indices, states_orig_dev[PLOT_STATE_IDX, :],
                    label=(sim_idx == 1) ? "DMDc Model" : "", # Label only for the first segment
                    color=:blue, linestyle=:dot, linewidth=1.2)
            end

            plot!(deviation_plot,
                title="Simulations Branching from Original Data Trajectory\n(Delay=$delay, State=$(original_state_label_str), Branch Sim Len=$(branch_sim_len))",
                xlabel="Global Time Index",
                ylabel="State $(original_state_label_str) Value",
                plot_titlefontsize=12, # Adjusted for potentially longer title
            ...
            figname_deviation = joinpath(OUTPUT_DIR, "dmdc_deviation_plot_delay$(delay)_state$(original_state_label_str)_baselen$(base_len)_simlen$(branch_sim_len).png")
            try
                savefig(deviation_plot, figname_deviation)
                println("\nDeviation comparison plot saved to $figname_deviation")
            catch e
                println("\nError saving deviation comparison plot $figname_deviation: $e")
            end
        end
    else
        println("No simulation start offsets defined for the deviation plot. Skipping.")
    end
