...
# ╔═╡ f27a5906-5673-46f5-a3bf-f0f8e0d22756
using OteraEngine, LinearAlgebra

# ╔═╡ c4775023-2257-42de-ab80-dc00a255939c
using WGLMakie, PlutoUI

# ╔═╡ 85a22cac-fe3e-4c42-be8b-8e0df4191641
cellsText = read("../heat-sim-freecad/0/C", String)

# ╔═╡ debf76c6-7b25-4df8-84b3-983c45ddc023
lines = split(cellsText, "\n")

# ╔═╡ 5ef3166d-53fd-4274-8499-97e3d064b082
begin
	cellStart = 0
	nCells = 0
...
# ╔═╡ 25f76879-c2e4-41b5-bb90-0ba7354fad1a
function lineToVec(line)
	coords = split(strip(line, ['(', ')']))
	return [parse(Float64, x) for x in coords]
end

# ╔═╡ 7b9d4a5e-cde0-48e1-8df1-b8d93a5c1ecf
cells = [lineToVec(line) for line in lines[cellStart:cellStart+nCells-1]]

# ╔═╡ 06c2a272-9d99-4229-980d-9022c6ae09cb
center = sum(cells)/size(cells,1)

# ╔═╡ dc8bc2d4-a509-4cb8-92e2-1e2aec073313
sum(cells)

# ╔═╡ 0abf61b1-922c-4657-8be2-63e2c2522328
# field(v) = [sin(v[2]), cos(v[1]), 0]
field(v) = cross(v-center, [0,0,-1])

# ╔═╡ 333142f1-6638-4dc6-82eb-3a72f2a9f16a
size(cells, 1), nCells

# ╔═╡ 468c6430-6f3e-4857-b4bd-1a6092c663cb
A = field.(cells)*10

# ╔═╡ 97af20f0-f14a-486b-825f-f4c57e62f822
begin
	A_filtered = (A)[1:100:size(A,1)]
	cells_filtered = cells[1:100:end]
	mag = 0.01
	arrows(
		map(cell -> cell[1], cells_filtered),
		map(cell -> cell[2], cells_filtered),
		map(cell -> cell[3], cells_filtered),
		map(a -> a[1]*mag, A_filtered),
		map(a -> a[2]*mag, A_filtered),
		map(a -> a[3]*mag, A_filtered),
		arrowsize=Vec3f(mag, 0.001, 0.01)
	)
end

# ╔═╡ 15e984f1-48ac-4598-820d-fc00837e3bda


# ╔═╡ bf3ca2fc-aea7-4a6f-8b42-5264d814a27b
cells[1]

# ╔═╡ a56810f6-653c-4996-a084-28d2514dddd7
begin
	template = read("./fieldsTemplate.mustache", String)
	data = Dict("nCells"=>nCells, "A"=>A)
	tmp = Template(template,path=false)
	render = tmp(init=data)
end

# ╔═╡ cb445dcf-8076-49ee-879c-9753780cd931
write(open("F_circleField", "w+"), render)
...
manifest_format = "2.0"
project_hash = "cfacd7dd7b9d3a9ffc5dfef0b58f816f99a06f4e"
...
git-tree-sha1 = "fd00935c531394ff30774cf8f7c89618597dd321"
